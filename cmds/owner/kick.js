  ...
  run: async (client, m) => {
    ...
    try {
      ...
      if (!jid.endsWith('@g.us')) {
        return reply(
          '❌ Este comando solo puede utilizarse dentro de grupos.'
        )
      }
      ...
      if (!botData?.admin) {
        return reply(
          '⚠️ Necesito ser administrador para expulsar usuarios.'
        )
      }
      ...
      const mentioned =
        ...
        m.message?.conversation?.match(/@(\d+)/g)?.map(
