...
async function seeCommands(dir = commandsFolder) {
  ...
  for (const fileOrFolder of items) {
    ...
    try {
      ...
      // Detectar categoría automáticamente desde la carpeta si no tiene category
      const relativePath = path.relative(commandsFolder, fullPath);
      const folderCategory = relativePath.includes(path.sep)
        ? relativePath.split(path.sep)[0]
        : 'general';
      const category = comando.category || folderCategory;
